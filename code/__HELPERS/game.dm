...
/proc/AnnounceArrival(mob/living/carbon/human/character, rank, datum/overmap_spawnable/handler)
	if(!SSticker.IsRoundInProgress() || QDELETED(character))
		return
	var/area/A = get_area(character)
	deadchat_broadcast(span_game(" has arrived on the [span_name("[handler.name]")] at [span_name("[A.name]")]."), span_game("[span_name("[character.real_name]")] ([rank])"), follow_target = character, message_type=DEADCHAT_ARRIVALRATTLE)
