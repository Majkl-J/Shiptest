...
// Antag rep used to mostly live here. Now it's dead.
...
/datum/game_mode/proc/generate_credit_text()
	...
	for(var/datum/overmap/location as anything in SSovermap.controlled_locations)
		var/datum/overmap_spawnable/handler = location.spawnable_handler
		if(length(handler.job_slots) == 0) //Way to find if it's a ruin-spawned ship or similar
			continue
		. += "<center><h1>The [capitalize(pick(GLOB.adjectives))] [handler.name]:</h1></center>"
		if(!length(handler.manifest))
			. += "<center><h2>It seems nobody crewed this ship today!<h2></center>"
		else
			for(var/name in handler.manifest)
				var/datum/job/their_job = handler.manifest[name]
